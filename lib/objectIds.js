...
function deviceIdFor(value) {
  ...
  const suffix = deviceId.match(/(?:^|_)0*([A-Z0-9]+)$/i);
  if (!suffix) {
    return deviceId;
  }

  return suffix[1].toUpperCase().padStart(3, "0");
}

function parseDeviceAliases(rows) {
  ...
  for (const row of rows) {
    ...
    if (row?.id !== undefined && name) {
      aliases.set(deviceIdFor(row.id), name);
    }
  }
